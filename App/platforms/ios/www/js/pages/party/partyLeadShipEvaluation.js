define(['app',
		'hbs!js/hbs/evaItem',
		'hbs!js/hbs/evaLeaderShipPaper',
		'hbs!js/hbs/evaLeaderShipPaperSeason',
		'hbs!js/hbs/evaLeaderShipPaperYear',
		'hbs!js/hbs/evaLeaderShipPaperSearch',
	...
	function initData(pageData) {
		//设置标题名称
		//$$('.partyEvaLeaderShipTitle').html(pageData.appName);
		//公用
		year = '';
		month = '';
		//月份
		...
		evaUserId =pageData.userId;
		startTime = pageData.startTime;
		endTime = pageData.endTime;
		evaType=pageData.type;
		evaKhpl=pageData.khpl;
		if(evaKhpl == 1){
			ajaxLoadContentYear();
		}else{
			ajaxLoadContent();
	...
	function clickEvent(page) {
		...
		$$(".evaLearderShipTime").on('click',function(){
			pickerDescribe.open();
			$("#picker-describe").val(year+'年 '+ month+'月');
		...
		$$(".evaLearderShipTimeSeason").on('click',function(){
		...
		$$(".evaLearderShipTimeYear").on('click',function(){
		...
		if(evaType == 1){
			...
			$$('#evaSearch').on('keyup', keyupAssessment);
		}else{
			$$('#evaSearch').on('keyup', keyupAssessmentByYear());
		}
		
		//点击tab标签
		//月份
		$$('.buttonShyk1').on('click',function(){
		...
		//季度
		$$('.buttonShykSeason1').on('click',function(){
			if($$('.evaTab').css('display') == 'none'){
				setTimeout(function(){
					searchAssessment1();
				},100);
		...
		//年份
		$$('.buttonShykYear1').on('click',function(){
			if($$('.evaTab').css('display') == 'none'){
				setTimeout(function(){
					searchAssessment1();
				},100);
...
//		});
	...
	function handleTopic(tpId) {
		...
		if(topicContent == '' || topicContent[0].state == 0){
			$$('.evaRightList ul').html("");
		}else{
...
//		//点击分数输入框
		...
		$$('.scoreSubmit').on('click',function(){
			...
			var evatopicIds1 = $$('.evaRightList').find('input[class="hiddenUS"]');
			var evatopicIds2 = $$('.evaRightList').find('input[class="hiddenRD"]');
			var indexMonth = 0;
			console.log(scores);
			$.each(userScores, function(index,item) {
				var inputScore = {
				  	topicId:0,
					reportUserId:evaUserId,
					userScore:0,
					totalScore:0,
					reportDetailId:0,
				}
				...
				console.log($$($$(evatopicIds2)[index]).val());
				inputScore.reportDetailId = parseInt($$($$(evatopicIds2)[index]).val());
				inputScore.userScore = parseInt($$($$(evatopicIds1)[index]).val());
				inputScore.topicId = parseInt($$($$(evatopicIds)[index]).val());
				inputScore.totalScore = parseFloat($$(item).val());
...
//				}
	...
	function handleTopicBySeason(tpIdSeason) {
		...
		if(topicContentSeason == '' || topicContentSeason[0].state == 0){
			$$('.evaRightListSeason ul').html("");
		}else{
...
//		if(topicContentYear == '' || topicContentYear[0].state == 0){
//			$$('.evaRightListYear ul').html("");
//		}else{
...
//		}
		if(topicContentYear == '' || topicContentYear[0].state == 0){
			...
			$$('.evaRightList ul').html(evaPaperYearTemplate(topicContentYear));
		}
		if(topicContentYear[0].hasScore){
		...
		$$('.scoreSubmitYear').on('click',function(){
			...
			var evatopicIds1 = $$('.evaRightListYear').find('input[class="hiddenUS"]');
			var evatopicIds2 = $$('.evaRightListYear').find('input[class="hiddenRD"]');
			var indexYear = 0;
			$.each(userScores, function(index,item) {
				var inputScore = {
				  	topicId:0,
...
//				}
	...
	function cancelAssessmentSearch(){
		...
		$$('.evaLeaderList').css('display','block');
		oldContent = '';
		$$('#evaSearch').val("");
...
//		});
	...
	function handleTopicBySearch(tpIdSearch) {
		...
		if(topicContentSearch == '' || topicContentSearch[0].state == 0){
			$$('.evaRightListSearch ul').html("");
		}else{
			$$('.evaRightListSearch ul').html(evaPaperSearchTemplate(topicContentSearch));
		}
		点击分数输入框
		if(topicContentSearch[0].hasScore){
		...
		$$('.scoreSubmitSearch').on('click',function(){
			...
			var evatopicIds1 = $$('.evaRightListSearch').find('input[class="hiddenUS"]');
			var evatopicIds2 = $$('.evaRightListSearch').find('input[class="hiddenRD"]');
			var indexSearch = 0;
			$.each(userScores, function(index,item) {
				var inputScore = {
				  	topicId:0,
