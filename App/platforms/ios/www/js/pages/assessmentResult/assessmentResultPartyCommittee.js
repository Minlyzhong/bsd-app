define(['app',
	'hbs!js/hbs/assessmentResultPartyCommittee',
	...
	function initData(page) {
		...
		str += '</div>';
		$$('.arpcDetailPage .kpi-a-list ul').html(str);
		CountyTime();
		loadPartyDeptHead();
		loadPartyDept(false);
		pushAndPull(page);
	...
	function CountyTime(){
		...
		$$(".rankTimeSeason").on('click',function(){
			...
			$$('.picker-3d .close-picker').on('click',function(){
				...
				str += '<div style="position: absolute;left: 45%;;font-size: 17px;">加载中...</div>';
				str += '</div>';
				$$('.arpcDetailPageSeason .kpi-a-list ul').html(str);
		...
		$$(".rankTimeYear").on('click',function(){
			...
			$$('.picker-3d .close-picker').on('click',function(){
				...
				str += '<div style="position: absolute;left: 45%;;font-size: 17px;">加载中...</div>';
				str += '</div>';
				$$('.arpcDetailPageYear .kpi-a-list ul').html(str);
	...
	function clickEvent(page) {		
		...
		//点击取消按钮
		$$('.arpcCloseBtn').click(function(){
			$('.assessmentPCSortBox').slideToggle(200);
			hideSearchList();
		});
		//点击确定按钮
		$$('.arpcSearchBtn').click(function(){
			//搜索框隐藏
			$('.assessmentPCSortBox').slideToggle(200);
			searchRank();
		});
	...
	function assessmentSortSearch(){
		...
		$$('.arpcDetailSearchPage').css('display','block');
	}
...
//		}
	...
	function handlePartyDeptHead(data) {
		$$('.doneNFA').html(data.completedTotal);
		$$('.uDoneNFA').html(data.unCompletedTotal)
	}
	/**
	 * 加载领导考核统计表头(季度)
	 */
	function handlePartyDeptHeadBySeason(data) {
		$$('.doneNFASeason').html(data.completedTotal);
		$$('.uDoneNFASeason').html(data.unCompletedTotal);
	}
	/**
	 * 加载领导考核统计表头(年份)
	 */
	function handlePartyDeptHeadByYear(data) {
		$$('.doneNFAYear').html(data.completedTotal);
		$$('.uDoneNFAYear').html(data.unCompletedTotal);
	}
	/**
	 * 加载领导考核统计表头(查询)
	 */
	function handlePartyDeptHeadBySearch(data) {
		$$('.doneNFASearch').html(data.completedTotal);
		$$('.uDoneNFASearch').html(data.unCompletedTotal);
	}
	...
	function handlePartyDept(data, isLoadMore) {
		if(data && data.length > 0) {
			if(isLoadMore) {
				$$('.arpcDetailPage .kpi-a-list ul').append(leaderTemplate(data));
			} else {
				$$('.arpcDetailPage .kpi-a-list ul').html(leaderTemplate(data));
			}
			//点击事件
			$$('.arpcDetailPage .assessLeaderContent').on('click', function() {
				var assessResultid = $$(this).data('id');
				app.myApp.getCurrentView().loadPage('assessmentResultPCDetail.html?assessResultid='+assessResultid+'&khpl=2&type=4&startDate='+rankMonthStartTime+'&endDate='+rankMonthEndTime);
			});
			if(data.length == 10) {
				loading = false;
			}
			...
			if(pageNo==1){
				$$('.arpcDetailPage .kpi-a-list ul').html('');
			}
	...
	function handlePartyDeptBySeason(data, isLoadMore) {
		if(data && data.length > 0) {
			if(isLoadMore) {
				$$('.arpcDetailPageSeason .kpi-a-list ul').append(leaderTemplate(data));
			} else {
				$$('.arpcDetailPageSeason .kpi-a-list ul').html(leaderTemplate(data));
			}
			//点击事件
			$$('.arpcDetailPageSeason .assessLeaderContent').on('click', function() {
				var assessResultid = $$(this).data('id');
//				if(count == 0) {
//					app.myApp.alert('该部门还没有考核记录');
//					return;
//				}
				app.myApp.getCurrentView().loadPage('assessmentResultPCDetail.html?assessResultid='+assessResultid+'&khpl=1&type=4&startDate='+rankSeasonStartTime+'&endDate='+rankSeasonEndTime);
			});
			if(data.length == 10) {
			...
			if(pageRankSeason==1){
				$$('.arpcDetailPageSeason .kpi-a-list ul').html('');
			}
		}
	}
	/**
	...
	function handlePartyDeptByYear(data, isLoadMore) {
		if(data && data.length > 0) {
			if(isLoadMore) {
				$$('.arpcDetailPageYear .kpi-a-list ul').append(leaderTemplate(data));
			} else {
				$$('.arpcDetailPageYear .kpi-a-list ul').html(leaderTemplate(data));
			}
			//点击事件
			$$('.arpcDetailPageYear .assessLeaderContent').on('click', function() {
				var assessResultid = $$(this).data('id');
//				if(count == 0) {
//					app.myApp.alert('该部门还没有考核记录');
//					return;
//				}
				app.myApp.getCurrentView().loadPage('assessmentResultPCDetail.html?assessResultid='+assessResultid+'&khpl=0&type=4&startDate='+rankYearStartTime+'&endDate='+rankYearEndTime);
			});
			if(data.length == 10) {
			...
			if(pageRankYear==1){
				$$('.arpcDetailPageYear .kpi-a-list ul').html('');
			}
		}
	}
	...
	function pushAndPull(page) {
		//加载更多
		var loadMoreContent = $$(page.container).find('.infinite-scroll.arpcDetailPage');
		...
		var loadMoreContent1 = $$(page.container).find('.infinite-scroll.arpcDetailSearchPage');
		...
		var loadMoreContent2 = $$(page.container).find('.infinite-scroll.arpcDetailPageSeason');
		...
		var loadMoreContent3 = $$(page.container).find('.infinite-scroll.arpcDetailPageYear');
