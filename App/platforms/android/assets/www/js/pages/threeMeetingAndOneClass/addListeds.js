define(['app',
	...
	function initData(pageData) {
		...
		if(selectType == 1){
			...
			$$('#textCenterSelectType').html('添加缺席人');
			$$('#textSelectType').html('缺席人：');
		}
...
//			deptId: 0,
		...
		function backAction() {
			...
			if(formType == 1){
				...
				require(['js/pages/threeMeetingAndOneClass/threeMeetingAndOneClassAdd'],function(threeMeetingAndOneClassAdd){
					if(selectType == 1){
						threeMeetingAndOneClassAdd.addLeaderBack(leaderList);
					}else{
						threeMeetingAndOneClassAdd.addAbsenteesBack(leaderList);
					}
				});
			}
	...
	function findDeptPeople(deptId, deptName) {
		app.myApp.getCurrentView().loadPage('showListeds.html?deptName=' + deptName + '&deptId=' + deptId + '&backPage=addListeds');
		return;
	}
