define(['app',
	'hbs!js/hbs/assessmentResultPCDetail'
	...
	var loading1 = true;
	//获取得分明细(历史记录)
	var searchResultDetailPath = app.basePath + '/mobile/partyAm/searchALeadertDetailByMonthAndUserId';
	//获取年份
	...
	function handleData(data, isLoadMore) {
		...
		if(data) {
			...
			$$('.threeMeetingsAndOneClassPaperDetailList .item-content').on('click', function() {
				...
				app.myApp.getCurrentView().loadPage('firstHandAssessDetail .html?assessId=' + id + '&title=' + title + '&score=' + score + '&name=' + name + '&reportState=' + reportState + '&userName=' + userName);
			});
			if(data.length == 10) {
				loading = false;
			}
