define(['app',
	'hbs!js/hbs/assessmentResultPersonal',
	...
	//个人完成情况
	var completedStatusPath = app.basePath + 'result/personalCompletedStatus';
	/*
	 * 参数
	 */
	var userPoint = 0;
	...
	function init(page) {
		initData(page);
		app.myApp.initPageInfiniteScroll($('.rankTabPersonal'));
		//加载分类
		//开始默认为DZZLX(党组织类型)
		sortCode = 'HKRYFL';
		findTypeByCode();
	...
	function findTypeByCode(){
		...
		$$(".sortTypeChoose").on('click',function(){
			...
			$$('.picker-3d .close-picker').on('click',function(){
				//把值赋到参数上
				assortmentPersonal = pickerSort.value[0];
			});
		});
	}
	...
	function CountyTime(){
		...
		$$(".rankTimeMonth").on('click',function(){
			...
			$$('.picker-3d .close-picker').on('click',function(){
				...
				str += '</div>';
				$$('.leaderDetailPage .kpi-a-list ul').html(str);
				rankMonthStartTime = year+'-'+month+'-1';
				rankMonthEndTime = year+'-'+month+'-31';
				loadPartyDeptHead();
				loadPartyDept(false);
			});
		...
		$$('.rankSelect .buttonShyk').on('click',function(){
			if($$('.rankTabPersonal').css('display') == 'none'){
				setTimeout(function(){
					searchRank();
				},100);
			}
	...
	function clickEvent(page) {		
		...
		//点击取消按钮
		$$('.arpCloseBtn').click(function(){
			$('.assessmentPersonalSortBox').slideToggle(200);
			hideSearchList();
		});
		//点击确定按钮
		$$('.arpSearchBtn').click(function(){
			//搜索框隐藏
			$('.assessmentPersonalSortBox').slideToggle(200);
			searchRank();
		});
	...
	function showSearchList(){
		$$('.rankTabPersonal').css('display','none');
		$$('.kpi-a-listSearchPersonal ul').html('');
		pageNo1 = 1;
		loading1 = true;
		queryByDeptName = ''
	...
	function hideSearchList(){
		$$('.rankTabPersonal').css('display','block');
		$$('.kpi-a-listSearchPersonal ul').html('');
		pageNo1 = 1;
		loading1 = true;
		queryByDeptName = ''
...
//		}
	...
	function handlePartyDeptHead(data) {
		$$('.personalDoneNFA').html(data.completedTotal);
		$$('.personalUDoneNFA').html(data.unCompletedTotal);
	}
	/**
	 * 加载领导考核统计表头(季度)
	 */
	function handlePartyDeptHeadBySeason(data) {
		$$('.personalDoneNFASeason').html(data.completedTotal);
		$$('.personalUDoneNFASeason').html(data.unCompletedTotal);
	}
	/**
	 * 加载领导考核统计表头(年份)
	 */
	function handlePartyDeptHeadByYear(data) {
		$$('.personalDoneNFAYear').html(data.completedTotal);
		$$('.personalUDoneNFAYear').html(data.unCompletedTotal);
	}
	/**
	 * 加载领导考核统计表头(查询)
	 */
	function handlePartyDeptHeadBySearch(data) {
		$$('.doneNFAPersonalSearch').html(data.completedTotal);
		$$('.uDoneNFAPersonalSearch').html(data.unCompletedTotal);
	}
	...
	function handlePartyDept(data, isLoadMore) {
		if(data && data.length > 0) {
			...
			$$('.leaderDetailPage .assessLeaderContent').on('click', function() {
				...
				app.myApp.getCurrentView().loadPage('assessmentResultPCDetail.html?assessResultid='+assessResultid+'&khpl=2&type=1&startDate='+rankMonthStartTime+'&endDate='+rankMonthEndTime);
			});
			if(data.length == 10) {
				loading = false;
			}
	...
	function handlePartyDeptBySeason(data, isLoadMore) {
		if(data && data.length > 0) {
			...
			$$('.leaderDetailPageSeason .assessLeaderContent').on('click', function() {
				var assessResultid = $$(this).data('id');
				app.myApp.getCurrentView().loadPage('assessmentResultPCDetail.html?assessResultid='+assessResultid+'&khpl=1&type=1&startDate='+rankSeasonStartTime+'&endDate='+rankSeasonEndTime);
			});
			if(data.length == 10) {
	...
	function handlePartyDeptByYear(data, isLoadMore) {
		if(data && data.length > 0) {
			...
			$$('.leaderDetailPageYear .assessLeaderContent').on('click', function() {
				var assessResultid = $$(this).data('id');
				app.myApp.getCurrentView().loadPage('assessmentResultPCDetail.html?assessResultid='+assessResultid+'&khpl=0&type=1&startDate='+rankYearStartTime+'&endDate='+rankYearEndTime);
			});
			if(data.length == 10) {
	...
	function handlePartySearchDept(data, isLoadMore) {
		if(data && data.length > 0) {
			if(isLoadMore) {
				$$('.kpi-a-listSearchPersonal ul').append(leaderTemplate(data));
			} else {
				$$('.kpi-a-listSearchPersonal ul').html(leaderTemplate(data));
			}
			//点击事件
			$$('.kpi-a-listSearchPersonal .assessLeaderContent').on('click', function() {
				var assessResultid = $$(this).data('id');
				console.log(khplSearch);
				console.log(typeSearch);
				console.log(rankSearchStartTime);
