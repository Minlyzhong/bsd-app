define(['app',
	...
	//模糊搜索部门
	var searchDeptPeoplePath = app.basePath + '/mobile/village/search';
	var oldContent = '';
	var deptList = [];
	var deptType = '';
	var childrenArr = [];
	...
	function handleData(data,elements) {
		...
		$$.each(deptData, function(index, item) {
			appendEle.find('ul').append(
				'<li class="li_' + index + '">' +
				'<a href="#" data-level="' + item.hasChild + '" data-deptId="' + item.id + '">' + item.label + '</a>' +
				'<div style="position: relative;float: right;bottom: 40px;right: 10px;">' +
				'<img src="img/notChooseDept.png" width="28" height= "28" data-deptId="' + item.id + '" data-deptName="' + item.label + '" />' +
				'</div>' +
				'</li>'
			);
			if(item.hasChild) {
				$('.u-vmenu .li_' + index).append('<ul>');
				var ind = 0;
				$$.each(item.children, function(_, cItem) {
					...
					if(cItem.hasChild){
						...
						$('.u-vmenu .li_' + index +'>ul').append('<li>' +
		...
		$(".u-vmenu").vmenuModule({autostart: true});
...
function handleChild2(data, element, currentEle) {
	...
	//当前元素点击打开
	currentEle.click();
	$('.u-vmenu').find('a').on('click', function() {
		var level = $$(this).data('level');
		var deptId = $$(this).data('deptId');
//			console.log(level);
		console.log(deptId);
