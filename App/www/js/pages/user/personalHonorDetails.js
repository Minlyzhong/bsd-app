	...
	function clickEvent(page) {
		...
		$$('.honorHandle').on('click', function() {
			...
			var popoverHTML = '<div class="popover" style="width: 40%;">' +
				...
				'<ul>' +
				'<li class="editHonor"><a href="#" class="editHonorButton" style="color:red;"><div><i class="icon icon-edit" style="margin-right: 7%;"></i><span>修改荣誉</span></div></a></li>' +
				'<li class="deleteHonor"><a href="#" class="deleteHonorButton" style="color:red;"><i class="icon icon-delete2" style="margin-right: 7%;"></i>删除荣誉</a></li>' +
				'</ul>' +
				'</div>' +
				'</div>' +
				'</div>'
...
//			}
			...
			$$('.editHonorButton').on('click',function(){
				app.myApp.getCurrentView().loadPage('personalHonorEdit.html?honorId='+honorId);
			});
	 ...
	 * 获取荣耀级别
	 */
	function loadHonorLevel(){
		app.ajaxLoadPageContent(honorLevelPath, {
			code: 'RYJB',
		}, function(result) {
			
			hovorLever = result.data;
			loadHonorDetail(honorId,hovorLever);
			loadpic(honorId);
